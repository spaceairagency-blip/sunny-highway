...
export const CONFIG = {
  // --- Road ---
  // Widened from the original 3×3.6m so the highway feels open rather
  // than cramped now that every car (traffic included) is the same
  // full-size model — more lanes, more width per lane, more shoulder.
  LANE_COUNT: 4,
  LANE_WIDTH: 4.6,
  ROAD_SEGMENT_LENGTH: 20,      // length of one repeating road tile
  ROAD_SEGMENTS_VISIBLE: 17,    // how many tiles kept alive ahead/behind
  ROAD_SEGMENTS_BEHIND: 3,      // of those, how many are kept BEHIND the car so
                                // the chase camera never overhangs empty space
                                // (this is what closes the white gap under the car)
  ROAD_TOTAL_WIDTH: 26,         // visual width of asphalt (extra shoulder)
  ...
  // Real car.glb half-dimensions (post-scale ~2.65m wide x ~4.2m long),
  // used for every collision/near-miss check now that traffic and race
  // opponents are the same model as the player instead of a smaller box.
  CAR_HALF_WIDTH: 1.3,
  CAR_HALF_LENGTH: 2.05,
  ...
  TRAFFIC_BLOCKADE_MAX_CHANCE: 0.38, // chance (at max difficulty) a spawn becomes a multi-lane blockade instead of a single car
  ...
  // --- Levels / themes ---
  // Goals are stretched way out compared to the original tuning (which
  // let a fast player reach level 28 in about 5 minutes): base goal and
  // step are both roughly 6x bigger, so early levels no longer fly by
  // in a couple of seconds and the full 50-level run is a real ~35-45
  // minute journey instead of ending in single digits of minutes.
  LEVEL_COUNT: 50,
  LEVEL_BASE_GOAL: 320,    // first goal is 320 meters (was 50)
  LEVEL_GOAL_STEP: 70,     // each subsequent goal grows by this many meters (was 17)
  THEME_TRANSITION_SECONDS: 1.6,

  // --- Coins / store economy ---
  COINS_PER_100M: 10,            // base payout, scaled by distance driven
  COINS_PER_LEVEL_CLEARED: 15,   // bonus per level reached in a run
  COINS_PER_NEAR_MISS: 1,        // rewards weaving through traffic
  COINS_RACE_WIN: 120,           // 1st place in race mode
  COINS_RACE_PODIUM: 60,         // 2nd or 3rd place
  COINS_RACE_FINISH: 25,         // anything else, as long as you finished
  COINS_NEW_BEST_BONUS: 50,
  ...
  STORAGE_COINS: 'sunnyhighway_coins',
  STORAGE_OWNED_CARS: 'sunnyhighway_owned_cars',
  STORAGE_SELECTED_CAR: 'sunnyhighway_selected_car',

  // --- Race mode (local vs AI opponents — no online/networked racing) ---
  RACE_OPPONENT_COUNTS: [2, 3, 4, 5, 6],
  RACE_FINISH_DISTANCE: 2200,        // meters to the finish line
  RACE_AI_BASE_SPEED_MIN: 30,        // m/s — each opponent's own baseline cruise speed
  RACE_AI_BASE_SPEED_MAX: 42,
  RACE_AI_MAX_SPEED: 50,             // hard cap even with rubber-banding
  RACE_AI_ACCEL: 12,                 // m/s^2 toward target speed
  RACE_AI_RUBBERBAND_PER_100M: 5,    // m/s of catch-up/ease-off per 100m gap to the player
  RACE_AI_RUBBERBAND_MAX: 11,        // m/s cap on the rubber-band nudge
  RACE_AI_LANE_CHANGE_COOLDOWN: 2.2, // seconds between an opponent's lane-change decisions
  RACE_AI_LOOKAHEAD: 26,             // meters an opponent scans ahead for a blocker
  RACE_AI_LATERAL_SPEED: 3.0,        // lateral units/sec while changing lanes
  RACE_MIN_FOLLOWING_GAP: 6.5,       // meters — anti-clip spacing between any two racers
  RACE_BUMP_SPEED_PENALTY: 9,        // m/s instantly shaved off on a racer-vs-racer bump
  RACE_COUNTDOWN_SECONDS: 3,
};

// --- Lane geometry helpers ---
// Shared by Road (lane divider lines), TrafficManager, and RaceManager so
// every system agrees on exactly where each lane sits, no matter how many
// lanes CONFIG.LANE_COUNT ends up being.
export function laneCenters(laneCount = CONFIG.LANE_COUNT, laneWidth = CONFIG.LANE_WIDTH) {
  const centers = [];
  const start = -((laneCount - 1) / 2) * laneWidth;
  for (let i = 0; i < laneCount; i++) centers.push(start + i * laneWidth);
  return centers;
}

// The painted dashed lines sit at the midpoint between each pair of
// adjacent lane centers (i.e. LANE_COUNT - 1 divider lines total).
export function laneDividerXs(laneCount = CONFIG.LANE_COUNT, laneWidth = CONFIG.LANE_WIDTH) {
  const centers = laneCenters(laneCount, laneWidth);
  const dividers = [];
  for (let i = 0; i < centers.length - 1; i++) dividers.push((centers[i] + centers[i + 1]) / 2);
  return dividers;
}

// A distinct palette for AI race opponents so a race grid never has two
// visually-identical cars (picked/shuffled at race start, skipping
// whichever color the player is currently driving).
export const RACE_OPPONENT_COLORS = [
  '#ff3355', '#3388ff', '#ffb500', '#33ff99', '#a855f7', '#ff7a1a',
  '#22d3ee', '#ff4fa0', '#7ce02c', '#f4f4f6', '#c81e3a', '#5a7a2f',
];
...
// =========================================================
// STORE CATALOG — the cars you can buy with coins.
//
// Every car is the same GLB (see CarModel) with its own paint job and
// its own handling profile, expressed as multipliers that PlayerCar
// applies on top of the base CONFIG physics. Keeping them as
// multipliers (rather than absolute numbers) means retuning the base
// car retunes the whole garage with it.
//
// `price: 0` + `starter: true` marks the car every player owns from
// the start, so the store is never a dead end for a new save.
// =========================================================
export const CARS = [
  {
    id: 'street-red', name: 'Street Runner', price: 0, starter: true,
    color: '#e5484d', blurb: 'The one you learned on. Balanced, forgiving, honest.',
    stats: { speed: 1.0, accel: 1.0, grip: 1.0, nitro: 1.0 },
  },
  {
    id: 'blue-comet', name: 'Blue Comet', price: 100,
    color: '#3388ff', blurb: 'Quicker off the line and noticeably sharper into gaps.',
    stats: { speed: 1.04, accel: 1.15, grip: 1.12, nitro: 1.0 },
  },
  {
    id: 'sun-taxi', name: 'Sunny Cab', price: 150,
    color: '#ffcc33', blurb: 'Heavier and slower to turn, but it holds a big NOS tank.',
    stats: { speed: 1.06, accel: 0.95, grip: 0.92, nitro: 1.4 },
  },
  {
    id: 'green-machine', name: 'Green Machine', price: 260,
    color: '#2fae60', blurb: 'Grippy enough to drift lane-to-lane without losing speed.',
    stats: { speed: 1.08, accel: 1.12, grip: 1.3, nitro: 1.1 },
  },
  {
    id: 'magic-violet', name: 'Magic Violet', price: 420,
    color: '#a855f7', blurb: 'Endless nitro regen. Keep the boost lit almost all run.',
    stats: { speed: 1.1, accel: 1.1, grip: 1.15, nitro: 1.9 },
  },
  {
    id: 'ghost-white', name: 'Ghost', price: 600,
    color: '#f4f4f6', blurb: 'Feather-light. Enormous acceleration, twitchy at full tilt.',
    stats: { speed: 1.14, accel: 1.4, grip: 1.2, nitro: 1.2 },
  },
  {
    id: 'midnight', name: 'Midnight', price: 850,
    color: '#14181d', blurb: 'Built for the night themes. Huge top end, real grip.',
    stats: { speed: 1.2, accel: 1.2, grip: 1.35, nitro: 1.3 },
  },
  {
    id: 'pro-gold', name: 'Pro Series', price: 1400,
    color: '#d4af37', blurb: 'The full-spec car. Fastest, sharpest, biggest tank.',
    stats: { speed: 1.28, accel: 1.45, grip: 1.45, nitro: 1.6 },
  },
];

export const STARTER_CAR_ID = CARS.find((c) => c.starter).id;

export function getCarById(id) {
  return CARS.find((c) => c.id === id) || CARS.find((c) => c.starter);
}
