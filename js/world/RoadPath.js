...
export class RoadPath {
  ...
  sample(distance) {
    // Negative distances (behind the start line) used to be clamped to 0,
    // which meant no road geometry could ever be built behind the start —
    // and since the chase camera sits ~8.5m BEHIND the car, the bottom of
    // the screen showed raw sky background instead of tarmac for the first
    // moments of every run (the "white gap under the car", with the lane
    // dashes of segment 0 poking out into it).
    //
    // The path always starts straight (heading 0 at the origin), so
    // extending it backwards is just a straight line down -Z. This is
    // exactly continuous with the generated path at distance 0, so the
    // seam is invisible.
    if (distance < 0) {
      return {
