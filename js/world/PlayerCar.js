...
export class PlayerCar {
  ...
  _addLights() {
    ...
    // these for night themes via setHeadlightIntensity().
    const headL = new THREE.PointLight(0xffffff, 0.15, 5);
    headL.position.set(-0.5, 0.5, 1.9);
    const headR = new THREE.PointLight(0xffffff, 0.15, 5);
    ...
    // Locate the shared "car_body" material (used by body/hood/trunk/etc.)
    // so livery color changes only need to touch one material instance.
    if (this.model) {
      this.model.traverse((child) => {
        if (!this._bodyMat && child.isMesh && child.material && child.material.name === 'car_body') {
          this._bodyMat = child.material;
          this._bodyMatBaseColor = child.material.color.clone();
        }
      });
    }
  }
  ...
  setHeadlightIntensity(t) {
    ...
    const intensity = 0.15 + t * 1.35;
    this._headlightGlow.forEach((l) => { l.intensity = intensity; });
  }
